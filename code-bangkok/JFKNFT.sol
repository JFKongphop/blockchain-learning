...
abstract contract ERC721 is IERC165, IERC721, ERC721Metadata, ERC1155Metadata_URI, ERC721Enumerable{
    ...
    mapping(address => mapping(address => bool)) _operatorAppovals; // owner => operator => allow(true / false)
    mapping(uint => address) _tokenApprovals; // tokenId => operator

    // meta data 
    string _name;
    string _symbol;
    mapping(uint => string) _tokenURLIs; // token => URIs
    ...
    mapping(uint => uint) _ownedTokenIndex; // tokenId => index
